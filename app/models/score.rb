class Score < ApplicationRecord
  def self.detail_graph(subject, newest)
    target_idols = where(subject: subject, datetime: newest).order(:rank).limit(3).pluck(:idol)
    idols = where(subject: subject, idol: target_idols).order(:idol, :datetime)
    idols = idols.group_by(&:idol)

    LazyHighCharts::HighChart.new('graph') do |f|
      f.title('得票数推移')
      f.xAxis(categories: datetimes(subject))
      f.yAxis([
        { title: { text: '総得票数' } },
      ...
      f.series(name: '1位と2位の差', data: diffs(subject), marker: { enabled: false }, yAxis: 1)
    end
  end
  
  scope :datetimes, -> (subject) do
    datetimes = where(subject: subject).order(:datetime).pluck(:datetime).uniq
